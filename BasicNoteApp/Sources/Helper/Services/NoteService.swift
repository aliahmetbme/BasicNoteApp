...
class NoteService {
    func createNote(title: String, note: String, completion: @escaping (Result<NoteResponseModel, ErrorResponse>) -> Void) {
    ...
    func deleteNote(note_id: Int, completion: @escaping (Result<DeletedNote, ErrorResponse>) -> Void) {
    ...
    func getAllNotes(page: Int = 1, completion: @escaping (Result<NoteResponseModel, ErrorResponse>) -> Void) {
    ...
    func getSpecificNote(note_id: Int, completion: @escaping ((Result<SpecificNoteResponseModel, ErrorResponse>) -> Void)) {
        let parameters: Parameters = ["note_id": note_id ]
        let token = UserDefaults.standard.value(forKey: "accesToken") as! String
        let headers : HTTPHeaders = ["Authorization": "Bearer \(token)" ]
    ...
    func updateNote(note: Note, completion: @escaping ((Result<SpecificNoteResponseModel, ErrorResponse>) -> Void)) {
    ...
    func createNote(note: UploadNoteModel, completion: @escaping ((Result<AddNoteResponseModel, ErrorResponse>) -> Void)) {
